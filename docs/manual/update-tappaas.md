...
description: TAPPaaS update scheduler for automated system updates
---
...
| `weekday` | Day of week (ignored for daily): `"Monday"` through `"Sunday"` |
| `hour` | Hour of day (0-23) when the update should run |
...
Use `update-cron.sh` to install the hourly cron job:
